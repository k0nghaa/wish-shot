...
- **Batch D** (2026-09-21 구현 완료, 실기기 검증은 사람): 브랜치 `feat/phase-7-onboarding`(A 머지된 dev에서 분기). 새 네이티브 모듈 없음(순수 JS — A-3의 `react-native-svg`·`@react-native-async-storage/async-storage` 위에 얹음). `tsc`·`expo lint` 통과.
  - **신규 `src/components/Onboarding/`**:
    ...
    - `CardMock.tsx`(`CaptureFlowMock`·`ShareSheetMock`): 첫 카드용 **코드 모션 그래픽** — "캡처 순간(피드+좌하단 캡처 썸네일) → 편집 화면(마크업·공유·완료 상단바) → 공유 시트 속 WishShot 강조" 3장면을 `setInterval`(2.2s) + reanimated 크로스페이드로 반복. **실제 스크린샷(인물·브랜드 포함)을 싣지 않고** 흐름만 재현(초상권/저작권 회피) — 에셋 불필요·토큰 기반·테마 자동 대응. reduce-motion 이면 마지막 장면(공유 시트)만 정적 표시. (사용자 결정: 코드 모션 그래픽으로 진행, 실제 스크린샷 PNG 교체는 실기기 확인 후 판단.)
...
- [ ] 첫 카드 **캡처 흐름 모션 그래픽**(캡처→편집→공유 시트 WishShot)이 3장면 반복으로 보인다(실제 스크린샷 PNG 교체 여부는 여기서 판단).
