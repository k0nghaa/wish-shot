...
- **Batch D** (2026-09-21 구현 완료, 실기기 검증은 사람): 브랜치 `feat/phase-7-onboarding`(A 머지된 dev에서 분기). 새 네이티브 모듈 없음(순수 JS — A-3의 `react-native-svg`·`@react-native-async-storage/async-storage` 위에 얹음). `tsc`·`expo lint` 통과.
  - **신규 `src/components/Onboarding/`**:
    ...
    - `CardMock.tsx`(`CaptureFlowMock`·`ShareSheetMock`): 첫 카드용 **코드 모션 그래픽** — "피드에서 캡처 → 좌하단 캡처 썸네일 **탭** → 편집 화면이 **올라옴** → 공유 아이콘 **탭** → 공유 시트가 **올라옴**"을 **하나의 연결된 흐름**으로 반복(4단계 `setInterval` 1.7s). 3개 독립 장면 크로스페이드가 아니라 **레이어를 쌓고**(피드/편집/공유) 편집·공유는 아래서 위로 슬라이드업, 각 상호작용 지점에 **탭 링 힌트**(pulse)를 얹어 인과를 잇는다. **실제 스크린샷(인물·브랜드 포함)을 싣지 않고** 흐름만 재현(초상권/저작권 회피) — 에셋 불필요·토큰 기반·테마 자동 대응. reduce-motion 이면 공유 시트 단계로 정적 표시. (사용자 결정: 코드 모션 그래픽으로 진행, 실제 스크린샷 PNG 교체는 실기기 확인 후 판단. 탭 링 좌표는 실기기 미세조정 전제.)
    ...
    - **카드 3장(문구는 실제 기능과 일치·앱 톤 단답)**: ① "스크린샷으로 담기" — "공유 시트에서 위시샷을 선택하면 스크린샷이 바로 담깁니다."(캡처 흐름 모션 그래픽) ② "AI가 자동 정리" — "제품명·가격·브랜드를 AI가 읽어 채우고 카테고리를 추천합니다."(`sparkles`) ③ "모아서 관리" — "카테고리로 정리하고, 링크를 저장하고, 원본은 앨범에서 정리합니다."(`square.grid.2x2`).
...
- [ ] 첫 카드 **캡처 흐름 모션 그래픽**이 하나의 연결된 흐름(캡처→썸네일 탭→편집 올라옴→공유 탭→공유 시트 올라옴)으로 자연스럽게 반복되고, 탭 링이 상호작용 지점에 얹힌다(좌표 미세조정 필요 시 기록. 실제 스크린샷 PNG 교체 여부도 여기서 판단).
