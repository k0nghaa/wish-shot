...
- **Batch D** (2026-09-21 구현 완료, 실기기 검증은 사람): 브랜치 `feat/phase-7-onboarding`(A 머지된 dev에서 분기). 새 네이티브 모듈 없음(순수 JS — A-3의 `react-native-svg`·`@react-native-async-storage/async-storage` 위에 얹음). `tsc`·`expo lint` 통과.
  - **신규 `src/components/Onboarding/`**:
    ...
    - `CardMock.tsx`(`ShareSheetMock`): 첫 카드용 **코드 데모 그래픽** — iOS 공유 시트를 흉내 낸 미니 목업(미리보기 썸네일 + 앱 아이콘 행에서 "위시샷" 타일 강조). 에셋 불필요·디자인 토큰만·테마 자동 대응. (사용자 결정: 일단 코드 목업으로 진행, 이후 실제 스크린샷 PNG 교체 여부는 실기기 확인 후 결정 — 교체 시 카드 `mock` 슬롯에 이미지 렌더만 갈아끼우면 됨.)
    - `OnboardingOverlay.tsx`: RN `Modal`(transparent). 단계머신 **cards → (transition) → category 코치마크 → (등록 폼 자동 open) → regImage 코치마크 → regRecent 코치마크 → 완료**. 카드 캐러셀은 `Animated.FlatList`(horizontal·pagingEnabled) + reanimated 스크롤 연동 도트, 첫 카드는 `ShareSheetMock`. 마지막 카드 "시작하기" → 카테고리 대상 측정(성공 시 코치마크, 실패 시 바로 등록 투어). **등록 투어**: `router.push('/register')`(빈 폼, 사진 미설정이라 OCR/AI·권한창 안 돎) 후 이미지 박스·"방금 캡처한 사진" 버튼을 재시도 측정(최대 8회×200ms — push 직후 레이아웃 지연 대비)해 코치마크. 종료(`finish`)는 우리가 연 폼을 `router.back()` 으로 닫고 `onDone`. "건너뛰기"·측정 실패는 어느 단계서든 안전하게 `finish`. push↔측정 사이엔 `transition`(딤+스피너)로 매끄럽게. **safe-area**: Modal 안에선 `SafeAreaView` top edge 가 0 으로 잡힐 수 있어 `useSafeAreaInsets` 로 상단/하단 인셋을 명시 적용(건너뛰기 헤더 가림 수정). **reduce-motion**(`AccessibilityInfo.isReduceMotionEnabled`)이면 Modal 전환·스크롤·도트 보간을 끄고 정적 표시.
    - **카드 3장(문구는 실제 기능과 일치·앱 톤 단답)**: ① "스크린샷으로 담기" — "공유 시트에서 위시샷을 선택하면 스크린샷이 바로 담깁니다."(공유 시트 목업) ② "AI가 자동 정리" — "제품명·가격·브랜드를 AI가 읽어 채우고 카테고리를 추천합니다."(`sparkles`) ③ "모아서 관리" — "카테고리로 정리하고, 링크를 저장하고, 원본은 앨범에서 정리합니다."(`square.grid.2x2`).
  ...
  - **결정/주의**: (a) 카테고리 코치마크 대상은 홈에서 항상 마운트되는 "카테고리 추가" 알약(FAB 는 '전체' 탭 포커스 때만 마운트라 launch 시 측정 불가). (b) 등록 폼 투어는 사용자가 + 버튼을 누를 필요 없이 **카테고리 코치마크 직후 자동으로** 폼을 열어 진행하며, 사진을 세팅하지 않아 실제 OCR/AI·권한 요청이 발생하지 않는다. (c) **RN `Modal` 이 네이티브 스택 모달 위에 겹치는지**(iOS 별도 UIWindow) 는 실기기 확인 필요 — 겹침·좌표·툴팁 위치는 **실기기 튜닝 전제**. (d) 새 네이티브 없음 — D 는 재빌드 불필요.
...
- [ ] 첫 카드 공유 시트 목업이 의도대로 보인다(실제 스크린샷 PNG 교체 여부는 여기서 판단).
- [ ] 카드 좌우 스와이프·도트 전환·"다음"/"시작하기" 버튼이 정상 동작한다.
- [ ] "시작하기" 후 **카테고리 코치마크** 구멍이 홈 "카테고리 추가" 알약에 맞는다(좌표 미세조정 필요 시 기록).
- [ ] "다음" → **등록 폼이 자동으로 열리고** 이미지 박스 코치마크 → "방금 캡처한 사진" 코치마크 순으로 뜬다(사진 선택창·권한창은 뜨지 않음).
- [ ] 코치마크가 등록 폼(네이티브 모달) **위에 정상적으로 겹쳐** 보이고 구멍/툴팁 좌표가 대체로 맞는다(RN Modal 겹침 확인).
