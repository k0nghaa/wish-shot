...
- **Batch D** (2026-09-21 구현 완료, 실기기 검증은 사람): 브랜치 `feat/phase-7-onboarding`(A 머지된 dev에서 분기). 새 네이티브 모듈 없음(순수 JS — A-3의 `react-native-svg`·`@react-native-async-storage/async-storage` 위에 얹음). `tsc`·`expo lint` 통과.
  - **신규 `src/components/Onboarding/`**:
    ...
    - `CardMock.tsx`(`CaptureFlowMock`): 첫 카드용 **코드 모션 그래픽** — "**캡처(플래시+좌하단 스크린샷 썸네일 등장)** → 썸네일 밝아짐 → **편집 화면이 올라옴** → 공유 아이콘 밝아짐 → **공유 시트가 올라옴** → WishShot 타일 밝아짐 → **위시 담기 폼이 올라옴**"을 **하나의 연결된 흐름**으로 반복(4단계 `setInterval` 1.7s). 레이어를 쌓고(피드/편집/공유/폼) 위 레이어가 아래서 위로 슬라이드업하며, 탭 대상은 **도형 자체가 밝아지는 펄스**(`Glow` — 흰색 오버레이 opacity, 별도 탭 링 아님)로 강조. 하단 단계 점(dots)은 제거. **실제 스크린샷(인물·브랜드 포함)을 싣지 않고** 흐름만 재현(초상권/저작권 회피) — 에셋 불필요·토큰 기반·테마 자동 대응. reduce-motion 이면 위시 담기 폼 단계로 정적 표시. (사용자 결정: 코드 모션 그래픽으로 진행, 실제 스크린샷 PNG 교체는 실기기 확인 후 판단.)
...
- [ ] 첫 카드 **캡처 흐름 모션 그래픽**이 하나의 연결된 흐름(캡처 플래시+썸네일 등장→편집 올라옴→공유 시트 올라옴→위시 담기 폼 올라옴)으로 자연스럽게 반복되고, 각 탭 대상 도형이 밝아지는 펄스로 강조된다(하단 점 없음. 실제 스크린샷 PNG 교체 여부는 여기서 판단).
