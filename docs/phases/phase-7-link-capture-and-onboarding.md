...
- **Batch D** (2026-09-21 구현 완료, 실기기 검증은 사람): 브랜치 `feat/phase-7-onboarding`(A 머지된 dev에서 분기). 새 네이티브 모듈 없음(순수 JS — A-3의 `react-native-svg`·`@react-native-async-storage/async-storage` 위에 얹음). `tsc`·`expo lint` 통과.
  - **신규 `src/components/Onboarding/`**:
    ...
    - **카드(문구는 실제 기능과 일치·앱 톤, 3장 모두 모션 그래픽)**: ① "스크린샷 바로 담기"(캡처→편집→공유시트→폼 연결 흐름 `CaptureFlowMock`) ② "AI가 자동 정리"("AI 분석 중…" 스피너 → 필드 순차 자동 채움 `AutoFillMock`) ③ "카테고리로 정리"(홈 카테고리 그리드가 순차로 채워지며 정리되는 흐름 `OrganizeMock`). 링크·앨범삭제·중복안내는 온보딩 헤드라인에서 제외(맥락/코치마크/저장 후 안내로 발견 — 사용자 결정).
