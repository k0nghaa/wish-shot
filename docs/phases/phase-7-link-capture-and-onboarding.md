...
- **Batch D** (2026-09-21 구현 완료, 실기기 검증은 사람): 브랜치 `feat/phase-7-onboarding`(A 머지된 dev에서 분기). 새 네이티브 모듈 없음(순수 JS — A-3의 `react-native-svg`·`@react-native-async-storage/async-storage` 위에 얹음). `tsc`·`expo lint` 통과.
  - **신규 `src/components/Onboarding/`**:
    ...
    - `RegisterCoachmarkTour.tsx`(신규): **등록 폼 위 코치마크 2스텝**(이미지 박스 → 방금 캡처한 사진). ⚠️ **등록 화면 안의 절대배치 오버레이**로 그린다(등록 화면이 직접 렌더) — 등록 폼이 네이티브 모달 "시트"라 화면 최상단과 오프셋이 있으므로, 대상을 `measureInWindow`(윈도우)로 얻은 뒤 **오버레이 루트의 윈도우 위치를 빼 "오버레이 로컬 좌표"로 보정**한다(구멍이 요소에 정확히 맞음 — 이전 RN Modal 방식은 시트 오프셋만큼 위로 어긋났음). Svg/딤 크기는 오버레이 `onLayout` 크기를 쓴다. 이미지 박스는 **정사각 구멍(shape='square', radius.lg, pad 12)**, 방금-캡처 버튼은 **알약 구멍**, 둘 다 흰 테두리 프레임. 재시도 측정(최대 10회×150ms). 측정 실패·건너뛰기는 어느 단계서든 안전 종료.
    - **카드(문구는 실제 기능과 일치·앱 톤, 3장 모두 모션 그래픽)**: ① "스크린샷 바로 담기"(캡처→편집→공유시트→폼 연결 흐름 `CaptureFlowMock`) ② "AI가 자동 정리"("AI 분석 중…" 스피너 → 필드 순차 자동 채움 `AutoFillMock`) ③ "카테고리로 정리"(실제 `CategoryCard` 처럼 **정사각 2×2 모자이크 + 하단 이름 오버레이** 카드가 순차로 나타나며 정리되는 흐름 `OrganizeMock`). 링크·앨범삭제·중복안내는 온보딩 헤드라인에서 제외(맥락/코치마크/저장 후 안내로 발견 — 사용자 결정). **각 카드 모션은 해당 카드가 활성일 때만 재생**(FlatList `extraData`+`active` prop, 활성 시 key 교체로 처음부터 재생 / 비활성·reduce-motion 은 정지·정적).
    - **코치마크 문구(3단계)**: 카테고리(홈) "여기서 카테고리를 추가하고 관리합니다." · 등록-이미지 "사진을 선택하면 AI가 제품 정보를 채웁니다. / 원하는 영역만 크롭해 다시 분석할 수 있습니다."(영역 크롭 버튼은 사진이 있어야 렌더돼 별도 스포트라이트 대신 이 툴팁에 흡수) · 등록-최근사진 "가장 최근에 저장한 사진을 바로 담을 수 있습니다."
