...
- 큰 이미지(`placeholder` 톤, radius ~14). **탭 → 풀스크린 뷰어**(원본 비율 `contentFit:"contain"`). 저장 원본은 업로드 시 **긴 변 1600px JPEG로 최적화 저장**(Phase 9 — egress·저장 절감). 그리드는 별도 400px 썸네일, 상세 뷰어만 이 최적화 원본을 비율 그대로 표시.
